...
public interface ProdutoRepository extends JpaRepository<Produto, Integer> {
    ...
    @Query("SELECT p FROM Produto p WHERE p.empresa.id = :idEmpresa AND p.bitStatus = 1")
    ...
    @Query("SELECT p FROM Produto p " +
            ...
            "OR UPPER(p.marca) LIKE UPPER(CONCAT('%', :nome, '%')))")
    List<Produto> findByNomeOrMarca(Integer idEmpresa, String nome);

    //buscar por todos os produtos de uma empresa por ordem alfabética
    @Query("SELECT p FROM Produto p " +
            "WHERE p.empresa.id = :idEmpresa " +
            "AND p.bitStatus = 1 " +
            "ORDER BY p.nome")
